...
static RISCVException ctr_smode(CPURISCVState *env, int csrno)
{
    if ((env->priv == PRV_M && riscv_cpu_cfg(env)->ext_smctr) ||
        (env->priv == PRV_S && riscv_cpu_cfg(env)->ext_ssctr)) {
        return RISCV_EXCP_NONE;
    }
...
static int rmw_cd_mhpmcounter(CPURISCVState *env, int ctr_idx, target_ulong *val,
...
static int rmw_cd_mhpmcounterh(CPURISCVState *env, int ctr_idx, target_ulong *val,
                               target_ulong new_val, target_ulong wr_mask)
{
    if (wr_mask != 0 && wr_mask != -1) {
        return -EINVAL;
...
static int rmw_cd_mhpmevent(CPURISCVState *env, int evt_index, target_ulong *val,
                            target_ulong new_val, target_ulong wr_mask)
{
    ...
    if (!wr_mask && val) {
        *val = env->mhpmevent_val[evt_index];
        ...
        env->mhpmevent_val[evt_index] =
            (new_val & wr_mask) | (env->mhpmevent_val[evt_index] & ~wr_mask);
        if (riscv_cpu_mxl(env) == MXL_RV32) {
            mhpmevt_val = mhpmevt_val |
                          ((uint64_t)env->mhpmeventh_val[evt_index] << 32);
        }
...
static int rmw_cd_mhpmeventh(CPURISCVState *env, int evt_index, target_ulong *val,
                             target_ulong new_val, target_ulong wr_mask)
{
                     ...
                     target_ulong new_val, target_ulong wr_mask)
{
    ...
    } else if (riscv_cpu_cfg(env)->ext_smcsrind || riscv_cpu_cfg(env)->ext_sscsrind) {
        return rmw_xireg_sxcsrind(env, csrno, isel, val, new_val, wr_mask);
    } else {
        return RISCV_EXCP_ILLEGAL_INST;
    }
                                    ...
                                    target_ulong val)
{
    ...
    uint64_t mask = MENVCFG_FIOM | MENVCFG_CBIE | MENVCFG_CBCFE | MENVCFG_CBZE | MENVCFG_CDE;
                                      ...
                                      target_ulong new_val)
{
    ...
    if (riscv_cpu_cfg(env)->ext_ssaia || riscv_cpu_cfg(env)->ext_smcsrind) {
                                    ...
                                    target_ulong new_val, target_ulong wr_mask)
{
    ...
    if (wr_mask & SCTRDEPTH_MASK) {
        uint64_t depth = get_field(new_val & wr_mask, SCTRDEPTH_MASK);

        if (depth > SCTRDEPTH_MAX) {
            depth = SCTRDEPTH_MAX;
        }

        new_val = set_field(new_val, SCTRDEPTH_MASK, depth);
    }
...
riscv_csr_operations csr_ops[CSR_TABLE_SIZE] = {
    ...
    [CSR_SCOUNTINHIBIT] = {"scountinhibit", scountinhibit_pred, read_scountinhibit,
                           write_scountinhibit,
                           .min_priv_ver = PRIV_VERSION_1_12_0 },
    ...
    [CSR_SCTRCTL]       = { "sctrctl",       ctr_smode, NULL, NULL,
                                rmw_sctrctl },
    [CSR_SCTRCTL]       = { "sctrdepth",       ctr_smode, NULL, NULL,
                                rmw_sctrdepth },
