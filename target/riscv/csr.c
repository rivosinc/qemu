...
static int validate_vm(CPURISCVState *env, target_ulong vm)
{
    vm &= 0xf;
    if (riscv_cpu_mxl(env) == MXL_RV32) {
        return valid_vm_1_10_32[vm] && (RISCV_CPU(env_cpu(env))->cfg.satp_vm & (1 << vm));
    } else {
        return valid_vm_1_10_64[vm] && (RISCV_CPU(env_cpu(env))->cfg.satp_vm & (1 << vm));
    }
}
...
static RISCVException sscdeleg(CPURISCVState *env, int csrno)
{
    ...
    /* All VS-mode access cause an exception */
    if (riscv_cpu_virt_enabled(env)) {
        return RISCV_EXCP_VIRT_INSTRUCTION_FAULT;
    }
    ...
    if (!is_hpmde_enabled(env) && ((csrno != CSR_SMCOUNTEREN))) {
        return RISCV_EXCP_ILLEGAL_INST;
    }
...
riscv_csr_operations csr_ops[CSR_TABLE_SIZE] = {
    ...
    [CSR_RTVEC]        = { "rtvec",        rcode, read_rtvec,        write_rtvec },
    [CSR_TRCM]         = { "trcm",         rcode, read_trcm,         write_trcm },
    [CSR_TRCS]         = { "trcs",         rcode, read_trcs,         write_trcs },
